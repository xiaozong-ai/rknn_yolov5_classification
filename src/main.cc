...
int main(int argc, char **argv) 
{
    ...
    // 读取图片
    cv::Mat origin_img = cv::imread(img_file, 1);
    if (!origin_img.data) {
        printf("opencv imread %s is fail!!!\n", img_file);
        return -1;
    }

    cv::Size crop_size(224, 224);
    cv::Mat crop_mat = centerCrop(origin_img, crop_size);
    cv::Mat transfer_mat = transfer(crop_mat);

    rknn_input inputs[1];
    memset(inputs, 0, sizeof(inputs));
    inputs[0].index = 0;
    inputs[0].type = RKNN_TENSOR_UINT8;
    inputs[0].size = transfer_mat.total() * transfer_mat.elemSize();
    inputs[0].fmt = RKNN_TENSOR_NHWC;
    inputs[0].buf = transfer_mat.data;
    ...
    size_t element_num = outputs->size / 4;         // 元素数量计算
    std::vector<float> output_vector(element_num);
